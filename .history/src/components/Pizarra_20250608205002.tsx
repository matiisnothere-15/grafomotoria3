...
const Pizarra: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    // limpiar canvas usuario también
    const canvasUsuario = canvasRef.current;
    if (canvasUsuario) {
      const ctxUsuario = canvasUsuario.getContext('2d');
      if (ctxUsuario) ctxUsuario.clearRect(0, 0, WIDTH, HEIGHT);
    }
    coordsRef.current = [];
